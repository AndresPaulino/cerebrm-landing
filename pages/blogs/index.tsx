...
const blogs = ({ blogs }: any) => {
  const pattern = {
    ...
    squares: [
      ...
      ...Array.from({ length: 10 }, () => [Math.floor(Math.random() * 20) - 10, Math.floor(Math.random() * 20) - 10]),
    ],
  };

  return (
    <Container
      title='Blogs | CEREB[RM]'
    ...
    >
      ...
      <div className='pb-4 pt-4 md:pb-20 md:pt-10 relative px-4'>
        <div className='mx-auto max-w-2xl sm:text-center pb-10 '>
          <h2 className='text-3xl font-medium tracking-tight text-gray-900'>Knowledge center at CEREB[RM]</h2>
          <p className='mt-2 text-lg text-gray-600'>
            Latest articles on web development and how a website can help you 10x your revenue.
          </p>
        </div>
      </div>
...
const BlogCard = ({ title, description, image, link, author, authorAvatar, publishedAt }: any) => {
  ...
  return (
    <Link href={link} className='relative  rounded-2xl p-4 transition duration-200'>
      <div className='flex flex-col cursor-pointer'>
        <div className='w-full aspect-w-16 aspect-h-10 bg-gray-100 rounded-lg overflow-hidden xl:aspect-w-16 xl:aspect-h-10 relative h-40 sm:h-72 md:h-64 '>
          <BlurImage src={image} alt='thumbnail' fill className='object-cover h-full w-full' />
        </div>
...
export async function getStaticProps() {
  ...
  let sortedBlogs = blogs.sort((a: any, b: any) => new Date(b?.publishedAt)?.getTime() - new Date(a?.publishedAt)?.getTime());

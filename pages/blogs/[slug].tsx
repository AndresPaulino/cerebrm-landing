...
export default function Blog({ mdxSource, frontMatter }: any) {
  return (
    <Container
      title={`${frontMatter.title} | CEREB[RM]`}
  ...
  return (
    <div className='my-10'>
      <div className='w-full h-52 md:h-96 rounded-lg relative flex justify-center mb-4 overflow-hidden'>
        <BlurImage src={frontMatter.image} alt='thumbnail' className='rounded-lg object-cover' layout='fill' />
      </div>
        ...
        <div className='flex flex-row justify-between space-x-2 w-full'>
          <div className='flex flex-row space-x-2 items-center'>
            <div className='border-2 border-gray-100  rounded-full flex items-center'>
              <BlurImage src={frontMatter.authorAvatar} width='20' height='20' className='rounded-full object-cover ' />
            </div>
            <span className='text-zinc-700 font-semibold'> {frontMatter.author}</span>
          </div>

          <span className='text-zinc-400 text-base'>{moment(frontMatter.publishedAt).format('Do MMM YYYY')}</span>
        </div>
      </div>
